...
class Tx_Cicbase_Service_GeolocationService {
	...
	protected function getCache() {
		if (TYPO3_UseCachingFramework) {
			...
			try {
				$cache = $GLOBALS['typo3CacheManager']->getCache('cictwit');
	...
	protected function geocode($address) {
		...
		if(TYPO3_UseCachingFramework && $this->cache->has($cacheKey)) {
			...
			if(TYPO3_UseCachingFramework && $out->location->lat && $out->location->lng) $this->cache->set($cacheKey,serialize($out),array(),3600);			
		}
		return $out;
	}
